...
def test_get_extra_fields(settings, user_api_client, user):
    settings.TGR_USER_EXTRA_FIELDS = {
        "yolo": ["rest_framework.fields.CharField", {'source': 'pk', 'read_only': True}],
    }
    configure()
...
def test_accounts_login(api_client, user):
    ...
    response = api_client.post(reverse('api-user-login'), data={"username": user.username, "password": "test"})
    assert response.status_code == 200
...
def test_accounts_login_error(api_client, user):
    ...
    response = api_client.post(reverse('api-user-login'), data={"username": user.username, "password": "asdasds"})
    assert response.status_code == 400

    assert response.json() == {'errors': {'non_field_errors': ['Unable to login with provided credentials.']}}
